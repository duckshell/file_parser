...
filepath = '/home/arno/gitPro/deepin-wine-ubuntu/deepin-wine-helper_1.2deepin0_i386/opt/deepinwine/tools/startbottle.exe'
#filepath = '/home/arno/gitPro/deepin-wine-ubuntu/deepin-libwine_2.18-12_i386/usr/lib/i386-linux-gnu/deepin-wine/fakedlls/xapofx1_3.dll'
pe_info = {}
...
IMAGE_IMPORT_BY_NAME  = [
    {'name': 'Hint'},
    {'name': 'Name','size':1,'count':0, 
        'format':lambda ss:reduce(lambda x1,x2:x1+x2 ,map(lambda x:str(x,encoding='ascii'),ss),''),
        'end':lambda: pe_info['Imports'][-1]['int'][-1]['IMAGE_IMPORT_BY_NAME']['Name'][-1] == b'\x00'
    },
]

IMAGE_IMPORT_BY_NAME2  = [
    {'name': 'Hint'},
    {'name': 'Name','size':1,'count':0, 
        'format':lambda ss:reduce(lambda x1,x2:x1+x2 ,map(lambda x:str(x,encoding='ascii'),ss),''),
        'end':lambda: pe_info['Imports'][-1]['iat'][-1]['IMAGE_IMPORT_BY_NAME']['Name'][-1] == b'\x00'
    },
]
...
IMAGE_THUNK_DATA2 = [
    ...
    {'name': 
        ...
        'slient':True,
        'stop':lambda:  
            pe_info['Imports'][-1]['iat'][-1]['ForwarderString'][-1]&(1<<7) == 1,
        'nest':IMAGE_IMPORT_BY_NAME2,
        'offset':lambda: offset(unpack(pe_info['Imports'][-1]['iat'][-1]['ForwarderString']))
    },
]
...
IMAGE_IMPORT_DESCRIPTOR = [
    ...
    {'name': 'iat', 'slient': True,
        'nest': IMAGE_THUNK_DATA2,
        'end': lambda: iat_end(), 'count':0,
        'offset':lambda:offset(unpack(pe_info['Imports'][-1]['FirstThunk']))
     },
]
...
def end_imports():
    return pe_info['Imports'][-1]['OriginalFirstThunk'] == b'\x00\x00\x00\x00'and pe_info['Imports'][-1]['TimeDateStamp'] == b'\x00\x00\x00\x00' and pe_info['Imports'][-1]['ForwarderChain'] == b'\x00\x00\x00\x00' and pe_info['Imports'][-1]['Name'][0] == b'\x00\x00\x00\x00' and pe_info['Imports'][-1]['FirstThunk'] == b'\x00\x00\x00\x00'
...
def parseStruct(f, header_struct, header, base_pos=None):
    ...
    for p in header_struct:
        ...
        if 'nest' in p:
            ...
            if count == 1:
                ...
                if 'type' in p and V(p, 'type') == 'string':
                    header[name]=(h,(readText(offset(unpack(h)))))
                else:
                    header[name] = h
        ...
        if 'format' in p:
            header[name] = p['format'](header[name])
...
def parse(f):
    ...
    # print('-----------')
    # parseImports()
    print(json.dumps(pe_info,cls=MyEncoder,indent=4))
    # for i in pe_info['Imports'][0]['int']:
